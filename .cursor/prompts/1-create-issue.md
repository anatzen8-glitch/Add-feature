...
- If Linear MCP is connected: Create the issue in Linear.
- Otherwise: Output a well-formatted issue (title, TL;DR, current vs expected, files, labels) that the user can copy to their issue tracker (Google Sheet, CSV, etc.). See SETUP-GUIDE.md for alternatives to Linear.
